    ...
    if args:
        if args[0] == 'json':
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "senorita_fbanned_users.json"
                update.effective_message.reply_document(document=output, filename="senorita_fbanned_users.json",
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "senorita_fbanned_users.csv"
                update.effective_message.reply_document(document=output, filename="senorita_fbanned_users.csv",
