...
def export_data(bot: Bot, update: Update, chat_data):
    ...
    f=open("senorita{}.backup".format(chat_id), "w")
    ...
    try:
        ...
        pass
    bot.sendDocument(current_chat_id, document=open('senorita{}.backup'.format(chat_id), 'rb'), caption="*Successfully imported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This  is specially made for notes.".format(chat.title, chat_id, tgl), timeout=360, reply_to_message_id=msg.message_id, parse_mode=ParseMode.MARKDOWN)
    os.remove("senorita{}.backup".format(chat_id)) # Cleaning file
